...
Phase 1.1: Test Location Analysis with 250m Grid

This script maps the spatial distribution of geotechnical test locations across the SESRO site
and adds a 250m analysis grid for enhanced visualization.
"""
...
def load_and_process_data(csv_path):
    ...
    # Data validation
    required_cols = ["Easting", "Northing", "TestType", "Depth", "AverageCu"]
    ...
    print("🎯 Processing unique test locations...")
...
def analyze_spatial_coverage(locations):
    ...
    print("Site boundaries (BNG coordinates):")
    ...
    print(f"Test density: {test_density:.1f} locations/km²")
...
def generate_analysis_grid(locations, grid_spacing=250):
    ...
    print(f"🔲 Generating {grid_spacing}m analysis grid...")
...
def assign_tests_to_grid(locations, grid_cells):
    ...
    print("📍 Assigning test locations to grid cells...")
    ...
    if len(cells_with_tests) > 0:
        print("Tests per cell statistics:")
...
def analyze_test_types(locations):
    ...
    print(
        f"Locations with multiple test types: {len(mixed_locations)} ({len(mixed_locations)/len(locations)*100:.1f}%)"
    )
...
def create_static_map_with_grid(
    ...
    """Create enhanced static map with grid overlay"""
    print("🗺️ Creating enhanced static map with 250m grid...")
...
def create_interactive_map_with_grid(locations, grid_cells, coverage_stats, output_dir):
    ...
    print("📱 Creating interactive map with grid overlay...")
...
def main():
    ...
    print("🎯 Ready for Phase 1.2: Cu Profile Analysis")
